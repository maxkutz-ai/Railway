...
Twilio webhook → POST https://app.receptionist.co/api/calls/inbound
  returns TwiML <Stream url="wss://aria-call-handler-production.up.railway.app/ws/call">
"""
...
def build_system_prompt(business_id: str, ctx: dict) -> str:
    """Build Aria's phone call system prompt from business config."""
    ...
    # Special note for the Receptionist.co sales demo line
    sales_note = ""
...
HUMAN (ALWAYS): If caller says "human", "agent", "representative", or presses 0 — say "Of course, let me have someone from our team call you right back. What's the best number to reach you?"
...
async def call_websocket(ws: WebSocket):
    ...
    logger.info(f"📞 Call connected | business={business_id} | sid={call_sid}")
    ...
    try:
        ...
        async with websockets.connect(
            ...
            await oai_ws.send(json.dumps({
                ...
                "session": {
                    "turn_detection": {
                        ...
                        "silence_duration_ms": 1200,  # 1.2s — natural pause for phone
                    },
            ...
            async def recv_twilio():
                ...
                async for message in ws.iter_text():
                    try:
                        ...
                        logger.warning(f"recv_twilio error: {e}")
                ...
                async for raw in oai_ws:
                    try:
                        ...
                        if event_type == "error":
                            ...
                            # Cancel current response when caller speaks
                            await oai_ws.send(json.dumps({"type": "response.cancel"}))

                    except Exception as e:
                        logger.warning(f"recv_openai error: {e}")
            ...
            for r in results:
                if isinstance(r, Exception) and not isinstance(r, asyncio.CancelledError):
                    logger.error(f"Stream error: {type(r).__name__}: {r}")
        ...
        # Save transcript and update call record
        if call_sid:
            sb = get_supabase()
            if sb:
                try:
                    ...
                    sb.table("calls").update({
                        ...
                        "duration_seconds":   None,  # Twilio sends this via status webhook
