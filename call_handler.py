...
9. SMS OPT-IN CAPTURE (TCPA / FCC 1:1 RULE — MANDATORY):
   Before sending ANY text message to a caller, you MUST first read the
   following verbal disclosure verbatim and wait for affirmative consent:

   "Before I text you, I just need to let you know — by giving me your
    mobile number, you're agreeing to receive a text from {business_name}
    with your [appointment confirmation / booking link / quote / invoice].
    Standard message and data rates may apply. You can reply STOP at any
    time to opt out, or HELP for support. Our full SMS terms are at
    receptionist dot co slash legal slash s-m-s dash terms. Is that okay?"

   Substitute the bracketed phrase with the specific scope of the message
   you are about to send (e.g. "appointment confirmation"). Wait for the
   caller to say "yes", "sure", "go ahead", "okay that works", or any
   clear affirmative. If the caller declines, hesitates, or asks
   questions, offer email instead — DO NOT send the SMS.

   Immediately after the caller affirms, call the log_sms_consent
   function with the caller's exact words. Do this BEFORE the SMS is
   sent. The consent record is the legal defense if a complaint arises.
10. PRICING DISCLOSURE (HOME SERVICES / TRADES VERTICAL — MANDATORY):
    When you quote ANY price, rate, or estimate from the business's
    price list during a booking conversation, you MUST immediately
    append in the same sentence: "but our technician will confirm the
    exact price when they see the job — there can be factors that
    change the final number." Do NOT leave a price standing alone.

    BAD:  "A new condenser unit starts around $4,200."
    GOOD: "A new condenser unit starts around $4,200, but our
           technician will confirm the exact price when they see
           the job — there can be factors like ductwork condition
           or refrigerant type that change the final number."

    For medical / wellness verticals, substitute: "but the final
    charge depends on what the provider determines during your visit."

    For services with fixed menu pricing (haircuts, car wash, basic
    massage), this rule does NOT apply — quote the menu price directly.

    Never commit to a price you quoted. If a caller says "you told me
    it would be $X", reply: "I gave you a ballpark from our price
    list, but the technician's estimate is the final number. I
...
# ─────────────────────────────────────────────────────────────────────────────
# OFFICEART_GUARDRAIL — Defense-in-depth for the user-only OfficeArt Services
# field per receptionist_contacts spec §4A.
#
# The CRM spec says:
#   "It is absolutely critical that this is a static, user-editable field.
#    It must not be populated, overwritten, or generated by the AI agent."
#
# Today, no Aria code path writes to industry_specific_data.office_service.
# This helper exists so that if any future code path (lead extractor, function
# call handler, special instructions writer, etc.) ever tries to write that
# key — accidentally or otherwise — the field is silently stripped before the
# Supabase update fires. The user's manual entry is preserved.
#
# Apply this helper at EVERY site that constructs an industry_specific_data
# dict for write. There is currently exactly ONE such site (the booking
# special_instructions writer around line ~1015), but this helper makes the
# guarantee explicit and grep-able.
#
# DO NOT remove this helper. If you find this code and think "we don't write
# office_service anywhere, this is dead code" — that is the point. It is a
# tripwire.
# ─────────────────────────────────────────────────────────────────────────────
PROTECTED_ISD_KEYS = frozenset({"office_service", "officeart_services"})

def strip_protected_isd_keys(isd: dict) -> dict:
    """Remove user-only fields from an industry_specific_data dict before
    writing it back to Supabase. Logs a warning if any protected key is
    found — that warning is the early signal that some code path is trying
    to write a field it shouldn't."""
    if not isinstance(isd, dict):
        return isd
    leaked = [k for k in PROTECTED_ISD_KEYS if k in isd]
    if leaked:
        logger.warning(
            f"OFFICEART_GUARDRAIL: stripping protected ISD keys before write: {leaked}. "
            f"This indicates an upstream code path is trying to write a user-only field. "
            f"Investigate and remove the upstream write."
        )
        for k in leaked:
            isd.pop(k, None)
    return isd
...
async def get_business_config(to_number: str) -> dict:
    ...
    try:
        r = sb.from_("settings_business").select(
            ...
            "outcomes_disclaimer_enabled,outcomes_disclaimer_text,self_service_portal_url"
...
async def handle_function_call(fn_name: str, fn_args: dict, business_id: str, to_number: str, call_sid: str = "", from_number: str = "") -> str:
    """
    ...
    ZIP 24 fix: from_number is now a parameter. Previously handle_dnc_request
    and handle_data_deletion_request referenced from_number from a non-existent
    closure scope, raising NameError that was silently swallowed by their
    try/except blocks. The DNC flag was therefore not actually being set in
    production. Now passed explicitly from the caller at the dispatch site.
    """
...
<Response>
    ...
    <Dial callerId="{to_number}">{transfer_number}</Dial>
                ...
                if special_instructions and contact_id:
                    try:
                        ...
                        if sb:
                            ...
                            # Defense in depth — never write user-only fields like office_service.
                            # See OFFICEART_GUARDRAIL block near scrub_pii for full rationale.
                            isd = strip_protected_isd_keys(isd)
    ...
    elif fn_name == "log_sms_consent":
        # ZIP 24 / TCPA Phase 2: record an affirmative SMS opt-in event.
        # Aria fires this immediately after the caller verbally affirms
        # the disclosure required by Identity Lock rule 9. The consent
        # row is the FCC 1:1 audit defense — without it, any SMS sent
        # later cannot be defended in a TCPA complaint.
        scope = (fn_args.get("scope") or "").strip()
        verbatim = (fn_args.get("verbatim_affirmation") or "").strip()
        if not scope or not verbatim:
            logger.warning(f"log_sms_consent missing required args: {fn_args}")
            return "Consent record incomplete. Tell the caller you'll send the message anyway and continue, but flag for follow-up."
        ...
        try:
            ...
            if not internal_secret:
                logger.error("log_sms_consent: RAILWAY_INTERNAL_SECRET not set on Railway — cannot record consent")
                return "Consent recording is not configured. Tell the caller you'll send the message and continue."

            CRM_BASE_URL = os.environ.get("CRM_BASE_URL", "https://app.receptionist.co")
            payload = {
                "business_id":          business_id,
                "contact_phone":        from_number,
                "contact_id":           contact_id,
                "scope":                scope,
                "verbatim_affirmation": verbatim[:200],
            ...
            async with httpx.AsyncClient(timeout=8.0) as client:
                resp = await client.post(
                    f"{CRM_BASE_URL}/api/sms/log-consent",
                    json=payload,
            ...
            if resp.status_code == 200:
                data = resp.json()
                logger.info(f"SMS consent logged: scope={scope} event_id={data.get('consent_event_id')} for {from_number}")
                return "Consent recorded successfully. Confirm to the caller: 'Perfect — sending that to you now.' Then proceed to send the SMS."
            elif resp.status_code == 401:
                logger.error(f"log_sms_consent 401: shared secret mismatch between Railway and CRM")
                return "Consent recording auth failed. Tell the caller you'll send the message and continue."
            else:
                logger.error(f"log_sms_consent failed {resp.status_code}: {resp.text[:200]}")
                return "Consent recording had a hiccup. Tell the caller you'll send the message and continue."

        except Exception as consent_err:
            logger.error(f"log_sms_consent exception: {consent_err}")
            return "Consent recording threw an error. Tell the caller you'll send the message and continue."
        ...
        async def receive_from_openai():
            ...
            async for raw in openai_ws:
                ...
                if event_type in ("response.audio.delta", "response.output_audio.delta") and data.get("delta"):
                    ...
                    result_text = await handle_function_call(fn_name, fn_args, business_id, to_number, call_sid=call_sid, from_number=from_number)
...
async def send_review_request(business_id: str, contact_id: str, contact_phone: str,
    ...
    try:
        ...
        # ── ZIP 24 / TCPA Phase 2: consent gate ───────────────────────────
        # Review requests are PROMOTIONAL sends per FCC 1:1 — caller is
        # not on the line, no transactional necessity. Must verify an
        # active (non-revoked) consent record exists for scope=
        # "review_request" before sending. Mirrors the fail-CLOSED
        # semantics of canSendSmsTo() in lib/smsConsentCheck.ts.
        try:
            digits = re.sub(r"\D", "", contact_phone)
            phone_last10 = digits[-10:] if len(digits) >= 10 else digits

            # Hard opt-out check first
            if contact_id:
                opt = supabase.table("contacts").select("sms_opted_out,pause_automations").eq("id", contact_id).maybe_single().execute()
                if opt.data and (opt.data.get("sms_opted_out") or opt.data.get("pause_automations")):
                    logger.info(f"send_review_request BLOCKED: contact {contact_id} opted out")
                    return False

            # Affirmative consent check (fail-CLOSED — promotional send)
            consent = supabase.table("sms_consent_events").select("id,granted_at").eq(
                "business_id", business_id
            ).eq(
                "scope", "review_request"
            ).ilike(
                "contact_phone", f"%{phone_last10}"
            ).is_(
                "revoked_at", "null"
            ).order("granted_at", desc=True).limit(1).execute()

            if not consent.data:
                logger.info(f"send_review_request BLOCKED: no consent record for review_request scope, phone={contact_phone}")
                return False

            logger.info(f"send_review_request consent verified: event_id={consent.data[0].get('id')}")
        except Exception as gate_err:
            # Fail-CLOSED for promotional sends — do not send if the check itself errors
            logger.error(f"send_review_request consent gate threw (fail-closed): {gate_err}")
            return False
        # ── End consent gate ──────────────────────────────────────────────
